    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            if (dx != 0 && dy != 0){
                ...
                float y1 = ev.getY();
//                Log.d("zlb", "===>x:"+x1+"\ty:"+y1);
                if (/*Math.abs(x1 - dx) > 4 && */Math.abs(y1 - dy) > 10){
                    return true;
                }
            }
            dx = ev.getX();
            dy = ev.getY();
        }else if (ev.getAction() == MotionEvent.ACTION_UP || ev.getAction() == MotionEvent.ACTION_DOWN){
            dx = 0;
            dy = 0;
        }
        return false;
...
//        设置控件水平居中
    ...
    private void replyView() {
        ...
        if (onScrollListener != null){
            onScrollListener.releaseUp(distance);
        }
    }
    private int l, t, oldl, oldt;
    @Override
