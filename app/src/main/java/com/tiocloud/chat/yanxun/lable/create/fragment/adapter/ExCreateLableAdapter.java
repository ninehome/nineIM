 ...
 * desc :
 */
public class ExCreateLableAdapter extends CreateLableAdapter {
    ...
    private void parseMultiModels(List<MailListResp.Friend> friends) {
        ...
        for (int i = 0, friendSize = friends.size(); i < friendSize; i++) {
            MailListResp.Friend friend = friends.get(i);

            // 过滤不显示自己
            if (currUid != null && currUid.equals(String.valueOf(friend.uid))) {
                continue;
            }
